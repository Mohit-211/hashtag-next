...
import {
    AllProductsApi,
    ProductsByParentCategoryApi,
    ProductCategoryApi,
    ProductsByGrandCategoryApi,
...
type SelectionType = "category" | "parent" | "brand";

interface CategoriesViewProps {
    // Populated by the [categoryId]/[categorySlug] and
    // [categoryId]/[categorySlug]/[parentId]/[parentSlug] route wrappers.
    // Left undefined on the root /categories route.
    initialCategoryId?: string;
    initialCategorySlug?: string;
    initialParentId?: string;
    initialParentSlug?: string;
    // Populated by the /categories/brand/[brandId]/[brandSlug] route wrapper.
...
const SESSION_STORAGE_KEY = "activeSelection"; // personal/internal bookkeeping
...
/**
 * Turn a category/parent/brand name into a URL-safe slug segment.
 * e.g. "Apparel & Uniforms" -> "apparel-uniforms"
 */
...
/**
 * Persists the current selection (category / parent-category / brand) to
 * sessionStorage — plain JSON, for internal bookkeeping only (not shown to
 * the user, not reflected anywhere in the UI). Passing `null` clears it.
 */
...
export default function CategoriesView({
    ...
    // ── URL-restore gate ─────────────────────────────────────────────────────
    // On initial load, if the route carries a category (and optionally parent)
    // path param, or a brand query param, we must let the restore effect
    // (below) resolve the real selection BEFORE the product-fetch effect
    // runs — otherwise a race fires two requests (one for the default "All"
    // state, one for the restored selection) and whichever response lands
    // last "wins", which could leave the wrong product set on screen even
    // though the heading/pill look correct.
    ...
    // Request-id guard: ensures an in-flight fetch whose response arrives late
    // can never overwrite the results of a newer fetch (e.g. race between the
    // default "All" fetch and the restored-category fetch on reload, or rapid
    // category switching).
    ...
    // change. Next.js can reuse this component instance across sibling
    // dynamic routes (it doesn't always unmount/remount), so leftover
    // `hoveredCatId` from a hover on the previous page could otherwise stick
    // around and make the subnav show the wrong category's parents after
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Restore selection from the route — path params for category/parent
    // (/categories/[categoryId]/[categorySlug]/...) and path params for
    // brand (/categories/brand/[brandId]/[brandSlug]) — once categories/
    // brands have loaded. Supports deep-linking / refresh.
    // ─────────────────────────────────────────────────────────────────────────
    useEffect(() => {
        if (!categories.length) return;

        if (initialBrandId && brandList.length) {
            const brand = brandList.find((b) => String(b.id) === String(initialBrandId));
            if (brand) {
        ...
        if (initialCategoryId) {
            ...
            if (cat) {
                ...
                if (initialParentId) {
                    const parent = cat.parent_categories?.find(
                        (p) => String(p.id) === String(initialParentId)
                    );
                    if (parent) {
        ...
        // Whether or not we found a match above, restoration has been
        // attempted — clear the gate so product fetching can proceed.
        setUrlRestoreAttempted(true);
    ...
    const fetchProducts = useCallback(
        async (
            ...
            // Claim a fetch id. If a newer fetch starts before this one resolves,
            // this response gets discarded instead of overwriting fresher data.
            const thisFetchId = ++fetchIdRef.current;

            try {
                setProductLoading(true);

                // Priority: brand > parent category > grand category > all.
                // Each of these now hits its own dedicated endpoint instead of a
                // single shared "AllProductsApi" with a filter param.
                let res: ProductApiResponse;

                if (brand) {
                    res = await AllProductsApi({
                        page: pageNumber,
                        limit: LOAD_MORE_LIMIT,
                        brand_id: brand.id,
                    });
                } else if (parent) {
                    res = await ProductsByParentCategoryApi({
                        page: pageNumber,
                        limit: LOAD_MORE_LIMIT,
                        category_id: parent.id,
                    });
                } else if (category?.id) {
                    res = await ProductsByGrandCategoryApi({
                        page: pageNumber,
                        limit: LOAD_MORE_LIMIT,
                        grand_category_id: category.id,
                    });
                ...
                // A newer fetch has since been kicked off — ignore this stale response.
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Initial load — brands + categories in parallel.
    // Guarded by a ref (not just the effect dependency array) so that even if
    // this effect re-runs — e.g. React StrictMode's intentional double-invoke
    // in dev, or the component re-rendering for other reasons — the actual
    // network calls only ever fire once per mounted instance.
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Fetch products whenever categories are ready OR the active filter/sort
    // changes. Single, merged effect — avoids duplicate/racing fetches.
    //
    // Gated on `urlRestoreAttempted`: if the route carries a category/parent
    // path param (or brand query param) on initial load, we wait for the
    // restore effect above to resolve the real selection before firing the
    // first product fetch, so we never fetch "All" and the restored category
    // back-to-back.
    // ─────────────────────────────────────────────────────────────────────────
    useEffect(() => {
        if (!categories.length) return;
    ...
    useEffect(() => {
        ...
        const run = async () => {
            try {
                ...
                if (next.id !== null) {
                    persistSelection("category", next.id, { name: next.name });
                    const slug = next.slug || slugify(next.name);
                    router.push(`/categories/${next.id}/${slug}`, { scroll: false });
                } else {
    ...
    const handleCategorySelect = (cat: GrandCategory) => {
        ...
        if (cat.id !== null) {
            persistSelection("category", cat.id, { name: cat.name });
    ...
    const handleParentSelect = (grandCat: GrandCategory, parent: ParentCategory) => {
        ...
        persistSelection("parent", parent.id, {
            name: parent.title,
            grandCategoryId: grandCat.id,
        });

        const grandSlug = grandCat.slug || slugify(grandCat.name);
        const parentSlug = parent.slug || slugify(parent.title);

        router.push(
            `/categories/${grandCat.id}/${grandSlug}/${parent.id}/${parentSlug}`,
            { scroll: false }
        );
    };

    // Brand selection now mirrors category/parent exactly: a real path
    // segment carrying id + readable slug — /categories/brand/[id]/[slug] —
    ...
    const handleBrandSelect = (brand: Brand) => {
        ...
        persistSelection("brand", brand.id, { name: brand.name });

        const slug = brand.slug || slugify(brand.name);

        router.push(`/categories/brand/${brand.id}/${slug}`, { scroll: false });
    };
                    ...
                    >
                        <div className="cat-subnav-inner">
                            ...
                            {parents.map((parent) => (
                                <button
                                    key={parent.id}
                                    className={`cat-tab-dropdown-item ${activeParent?.id === parent.id ? "active" : ""
                                        }`}
                                    onClick={() => {
                                        setHoveredCatId("none");
                ...
                <div className="cat-topbar-left">
                    ...
                    <span className="cat-count">
                        ...
                        {productGridLoading && (
                            <span
                                className="spinner"
                                style={{ width: 12, height: 12, marginLeft: 8, verticalAlign: "middle" }}
                            />
                        )}
                    </span>
