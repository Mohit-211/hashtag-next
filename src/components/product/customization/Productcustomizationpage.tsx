...
import { ProductDetailApi, ProductDetailGuestApi } from "@/api/operations/product.api";
...
interface Product {
  ...
  categories: any[];
  parent_categories?: any[]; // 👈 add this
...
/* ── Print locations per garment view ── */
// T-Shirt: Front, Back, Left Sleeve, Right Sleeve
// Hoodie:  Front, Back, Side
// Hat:     Front, Back, Side
// Polo:    Front, Back, Side
type GarmentType = "tshirt" | "hoodie" | "hat" | "polo";
interface GarmentView {
  key: string;       // location ID sent to API
...
const GARMENT_VIEWS: Record<GarmentType, GarmentView[]> = {
  tshirt: [
    ...
    },
    {
      ...
      hotspots: [
        { id: "FULL_BACK", label: "Full Back", top: "44%", left: "50%" },
      ],
    },
    {
      key: "LEFT_SLEEVE",
      ...
      hotspots: [
        { id: "SLEEVE_LEFT", label: "L. Sleeve", top: "30%", left: "30%" },
      ],
    },
    {
      key: "RIGHT_SLEEVE",
      ...
      hotspots: [
        { id: "SLEEVE_RIGHT", label: "R. Sleeve", top: "30%", left: "70%" },
      ],
    },
  ],
  hoodie: [
    ...
    },
    {
      ...
      hotspots: [
        { id: "SLEEVE_LEFT", label: "Sleeve", top: "35%", left: "50%" },
      ],
    },
  ],
  hat: [
    {
      ...
      hotspots: [
        { id: "HAT_FRONT", label: "Front", top: "40%", left: "50%" },
      ],
    },
    {
      key: "BACK",
      ...
      hotspots: [
        { id: "HAT_BACK_ARCH", label: "Back Arch", top: "40%", left: "50%" },
      ],
    },
    {
      key: "SIDE",
      ...
      hotspots: [
        { id: "HAT_SIDE", label: "Side", top: "40%", left: "50%" },
      ],
    },
  ],
  polo: [
...
/* All possible print locations (for API payload label lookup) */
...
/* ── Known apparel color-name → hex fallback map ──
   Used only when a variant has no usable color_code from the backend.
   Extend this as new color names show up in your catalog. */
...
/* Validate that a string is a CSS color the browser will actually render.
   Falls back to "true" during SSR (no CSS.supports available) so this never
   throws server-side; the real check runs client-side where it matters. */
...
/* Resolve a single color token (hex, rgb(), or name) to something paintable.
   Tries the raw value first, then a normalized hex fallback if we recognize
   the apparel color name, then finally a neutral gray so nothing renders blank. */
const resolveColorToken = (raw: string): string => {
  ...
  return "#d1d5db"; // neutral gray placeholder — better than an invisible swatch
};
function drawCanvas({
...
export default function ProductCustomizationPage({ productId }: Props) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      const res = token
        ? await ProductDetailApi(String(productId))
        : await ProductDetailGuestApi(String(productId));
  ...
  const garmentType = useMemo<GarmentType>(() => {
    ...
    // Extract all parent category titles exactly
    const parentCategories: { id: number; title: string }[] = (product as any).categories
      ?.flatMap((c: any) => c.parent_categories ?? []) ?? [];
    const hasParent = (title: string) =>
      parentCategories.some(
        (p) => p.title.toLowerCase() === title.toLowerCase()
      );
    if (hasParent("Hats") || hasParent("Caps") || hasParent("Headwear") || hasParent("Beanies"))
      return "hat";
    if (hasParent("Hoodies") || hasParent("Hoodies/Sweatshirts") || hasParent("Sweatshirts"))
      return "hoodie";
    if (hasParent("Polos"))
      return "polo";
    if (hasParent("Jackets") || hasParent("Vests") || hasParent("Outerwear"))
      return "hoodie"; // Front/Back/Side tabs fit jackets/vests
    return "tshirt";
  }, [product]);
  const garmentViews = useMemo(() => GARMENT_VIEWS[garmentType], [garmentType]);
  /* Active view tab (Front / Back / Side / Left Sleeve / Right Sleeve) */
  const [activeViewIdx, setActiveViewIdx] = useState(0);
  const activeView = garmentViews[activeViewIdx] ?? garmentViews[0];
  /* Reset view tab when garment type changes */
  useEffect(() => { setActiveViewIdx(0); }, [garmentType]);
  const colors = useMemo(() => {
    if (!product) return [];
    const map = new Map<string, { color: string; color_code: string }>();
    product.variants.forEach((v) => {
      if (!map.has(v.color)) {
        console.log(v.color_code, "=====")
        const raw = v.color_code?.trim() || "";
        // Resolve each comma/slash-separated token to a guaranteed-valid CSS color.
        // This is what feeds the swatch render below — never store an unverified
        // guess (like the first word of the color name) here.
        const code = raw
          ? raw.split(/[\/,]/).map(resolveColorToken).join(",")
          : resolveColorToken(v.color.split(/[\/,\s]+/)[0] || "");
        map.set(v.color, { color: v.color, color_code: code });
      }
    });
  ...
  const displayImage = useMemo(() => {
    ...
    if (!primary) return product.attachments?.[0]?.file_uri || null;
    return primary.file_uri?.startsWith("http")
      ? primary.file_uri
      : primary.file_name?.startsWith("http")
        ? primary.file_name
        : `${BASE_URL}${primary.file_uri}`;
  }, [product, selectedColor]);
  /* ── Print locations ── */
  ...
  const wishlistItem = wishlist.find(i => i.product_id === productId);
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    drawCanvas({
      ctx, size: CANVAS_SIZE,
      productImg, logo: logoImg,
  ...
  const isOverLogo = (pt: { x: number; y: number }) =>
    !!(logoImg &&
      pt.x >= logoPos.x && pt.x <= logoPos.x + logoSize &&
  ...
  const updateOrderRow = (id: number, field: keyof OrderRow, value: any) =>
    setOrderRows(prev => prev.map(r => {
      ...
      if (field === "color") updated.variantId = ""; // reset size on color change
      return updated;
    }));
  const saveOrderRows = () => {
    const valid = orderRows.filter(r => r.color && r.variantId && r.qty >= 1);
    // Build variantQty map for compatibility with rest of the page
    ...
    // Also update selectedColor to first row's color if unset
  ...
  /* ── Payload builder ── */
  const buildPayload = () => ([{   // ✅ array, not object
  ...
  const getBlob = (): Promise<Blob | null> =>
    new Promise((res) => {
      ...
      canvas.toBlob(
        (blob) => {
          console.log("Canvas blob result:", blob, "size:", blob?.size);
          res(blob);
        },
        "image/png",
        1
  ...
  const handleAddToCart = async () => {
    ...
    console.log("Blob captured:", blob, "size:", blob?.size);
    canvasBlobRef.current = blob;  // ✅ set ref immediately (synchronous)
    setCanvasBlob(blob);           // keep state in sync too
  ...
  const handleOpenPreview = () => {
    ...
    try {
      ...
      off.width = CANVAS_SIZE * 2;
      off.height = CANVAS_SIZE * 2;
    ...
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-gray-100 shadow-sm">
        <div className="container max-w-6xl mx-auto px-4">
          <div className="flex items-center gap-3 py-3.5">
            <Link href={`/product/${productId}`} className="flex items-center gap-1.5 text-xs font-medium text-gray-400 hover:text-black transition-colors">
            ...
            <div className="ml-auto hidden md:flex items-center gap-1.5">
              {REQUIREMENTS.map(r => (
                <span key={r.key} className={cn("flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full border", r.done ? "bg-[#F5D800]/15 text-[#b89000] border-[#F5D800]/30" : "bg-gray-100 text-gray-400 border-gray-200")}>
      ...
      <div className="bg-white border-b border-gray-100 py-6">
        <div className="container max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-sm text-gray-500">Select color, sizes, print location & method — then add your design</p>
        </div>
      </div>
      {/* ── Main Layout ── */}
      <div className="container max-w-6xl mx-auto px-4 py-8">
        <div className="grid xl:grid-cols-[1fr_380px] gap-6">
          {/* ═══════════════════════ LEFT ═══════════════════════ */}
          <div>
            <SectionCard step={1} title="Choose Print Location" subtitle="Select a view, then tap the hotspot on the garment to choose your print location." status={selectedLocations.length > 0 ? "done" : "required"}>
              {/* ── View Tab Bar ── */}
            ...
            <SectionCard step={2} title="Decoration Method" subtitle="Choose how your design will be applied to the garment." status={selectedMaterial ? "done" : "required"}>
            ...
            >
              ...
              <div className="space-y-2 mb-3">
                {orderRows.map((row) => {
                  const sizesForColor = product?.variants.filter(v => v.color === row.color && v.stock > 0) ?? [];
                  // color_code on `colors` entries is already resolved to guaranteed-valid
                  // CSS color tokens (see the `colors` useMemo above), so no extra
                  // validation is needed here — just split multi-tone codes and paint.
                  const rawCode = colors.find(c => c.color === row.color)?.color_code || "#d1d5db";
                  const codes = rawCode.split(",").map(c => c.trim()).filter(Boolean);
                  const swatchBackground = codes.length > 1
                    ? `linear-gradient(135deg, ${codes.join(",")})`
                    : codes[0] || "#d1d5db";
                  return (
                    <div
                      key={row.id}
                      className="grid gap-2 items-center px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50"
                    ...
                    >
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="h-9 rounded-xl border border-gray-200 bg-white px-8 text-xs font-semibold text-gray-700 outline-none focus:border-[#F5D800] cursor-pointer appearance-none w-full"
                        >
                        ...
                        {row.color && (
                          <div
                            className="absolute left-2.5 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full border border-gray-300 shadow-sm flex-shrink-0 pointer-events-none"
                      ...
                      <input
                        ...
                        min={1}
                        value={row.qty}
                        onChange={e => updateOrderRow(row.id, "qty", Math.max(1, parseInt(e.target.value) || 1))}
                        className="h-9 rounded-xl border border-gray-200 bg-white text-center text-xs font-bold text-gray-900 outline-none focus:border-[#F5D800]"
                      />
                      {/* Size dropdown */}
                      <select
                        value={row.variantId}
                        ...
                        disabled={!row.color}
                        className="h-9 rounded-xl border border-gray-200 bg-white px-3 text-xs font-semibold text-gray-700 outline-none focus:border-[#F5D800] cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        <option value="">— choose size —</option>
                        {sizesForColor.map(v => (
                          <option key={v.id} value={v.id}>{v.size_details?.name || v.size}</option>
                        ))}
                      </select>
                      {/* Remove button */}
                      <button
                        ...
                        className="w-9 h-9 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 hover:border-red-200 hover:text-red-400 hover:bg-red-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                      >
                        <X size={13} />
                      </button>
                    </div>
            ...
            <SectionCard
              ...
              subtitle="Optionally upload a logo or add custom text. Drag to reposition on the canvas."
              status="optional"
            >
                      ...
                      {([
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            activeTab === key
                              ? "text-gray-900 border-[#F5D800] bg-[#FFFBEA]"
                    ...
                    <div className="p-5 space-y-4">
                        ...
                        <>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              r.onloadend = () => {
                                ...
                                const scale = productImg
                                  ? Math.min(CANVAS_SIZE / productImg.naturalWidth, CANVAS_SIZE / productImg.naturalHeight)
                                  : 1;
                                const drawW = productImg ? productImg.naturalWidth * scale : CANVAS_SIZE;
                                const drawH = productImg ? productImg.naturalHeight * scale : CANVAS_SIZE;
                                const offsetX = (CANVAS_SIZE - drawW) / 2;
                                const offsetY = (CANVAS_SIZE - drawH) / 2;
                                setLogoPos({
                                  x: offsetX + (drawW - defaultSize) / 2,
                                  y: offsetY + (drawH - defaultSize) / 2,
                                });
                              };
                          ...
                          {!logoImg ? (
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => setLogoRotation(p => (p - 15 + 360) % 360)} className="flex-1 h-9 rounded-xl border border-gray-200 flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50"><RotateCcw size={13} /> Left</button>
                                <button onClick={() => setLogoRotation(p => (p + 15) % 360)} className="flex-1 h-9 rounded-xl border border-gray-200 flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50"><RotateCw size={13} /> Right</button>
                              </div>
                            </div>
                          )}
                        </>
                      ...
                      {activeTab === "text" && (
                        <>
                          ...
                          <div className="flex gap-2">
                            {([
                              { label: "B", active: textBold, toggle: () => setTextBold(p => !p), cls: "font-black" },
                              { label: "I", active: textItalic, toggle: () => setTextItalic(p => !p), cls: "italic" },
                              { label: "Shadow", active: textShadow, toggle: () => setTextShadow(p => !p), cls: "" },
                            ] as const).map(({ label, active, toggle, cls }) => (
                              <button
                                key={label}
                                onClick={toggle}
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => setTextRotation(p => (p - 15 + 360) % 360)} className="flex-1 h-9 rounded-xl border border-gray-200 flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50"><RotateCcw size={13} /> Left</button>
                            <button onClick={() => setTextRotation(p => (p + 15) % 360)} className="flex-1 h-9 rounded-xl border border-gray-200 flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50"><RotateCw size={13} /> Right</button>
                          </div>
                          {customText.trim() && (
                            <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">Preview</p>
                              <p style={{ fontFamily, fontSize: Math.min(textSize, 28), color: textColor, fontWeight: textBold ? 700 : 400, fontStyle: textItalic ? "italic" : "normal", textShadow: textShadow ? "2px 2px 4px rgba(0,0,0,0.3)" : "none", opacity: textOpacity, lineHeight: 1.4 }}>
                                {customText}
                              </p>
                            </div>
                          )}
            ...
            {/* STEP 1+2 — COLOR + SIZE + QUANTITY rows (MOVED TO LAST) */}
          </div>
          {/* ═══════════════════════ RIGHT: Order Summary ═══════════════════════ */}
          <div className="xl:sticky xl:top-[60px] self-start">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                {/* Print details */}
                {(selectedLocations.length > 0 || selectedMaterial) && (
                  <div className="rounded-xl border border-gray-100 overflow-hidden">
                    <div className="bg-gray-50 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400 border-b border-gray-100">
                      Print Details
                    </div>
                      ...
                      {printPrice !== null && (
                        <div className="flex items-center justify-between pt-1.5 border-t border-gray-100">
                          <span className="text-xs text-gray-500">
                            Decoration{selectedMaterial === "embroidery" && totalQty <= 11
                              ? " (incl. $35 digitizing)"
                              : selectedLocations.length > 1 ? ` (${selectedLocations.length} loc.)` : ""}
                          </span>
                ...
                <div className="rounded-xl bg-gray-900 text-white p-4">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xl font-black text-[#F5D800]">
                      {totalQty > 0
                        ? selectedLocations.includes("OVERSIZED_BACK") && selectedMaterial === "embroidery"
                          ? `$${(basePrice * totalQty).toFixed(2)} + quote`
                          : `$${estimatedTotal.toFixed(2)}`
                        : "—"}
                    </span>
                  </div>
                  {totalQty > 0 && printPrice !== null && (
                ...
                {/* Tier hint */}
                {selectedMaterial && totalQty > 0 && totalQty < 144 && (
                ...
                {/* Dev payload preview — remove in production */}
                {allMet && (
                  <details className="text-[10px] text-gray-400">
                    <summary className="cursor-pointer font-semibold hover:text-gray-600">View API payload</summary>
                    <pre className="mt-2 p-2 bg-gray-50 rounded-lg border border-gray-200 text-[9px] overflow-auto max-h-40 text-gray-500">
      ...
      {product && (
        <AddToCartModal
          ...
          printPricePerPiece={
            printPrice && totalQty > 0
              ? (selectedMaterial === "embroidery" && totalQty <= 11
                ? (printPrice - 35) / totalQty
                : printPrice / totalQty)
              : 0
          }
          digitizingFee={selectedMaterial === "embroidery" && totalQty <= 11 ? 35 : 0}
