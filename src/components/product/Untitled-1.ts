    ...
    <div className="mt-3 rounded-xl border border-gray-200 overflow-hidden">
            ...
            <tr>
              ...
              {DTF_PRICES.map((p, i) => (
                <td key={i} className={cn(
                  ...
                  activeTier === i ? "text-[#b89000] font-bold text-sm bg-[#F5C400]/5" : "text-gray-500"
                )}>
                  ${p}.00
                </td>
              ))}
      ...
      <div className="bg-gray-50 px-4 py-2 border-t border-gray-200">
        <p className="text-[10px] text-gray-400">
          ...
          <span className="font-semibold text-[#b89000]">${DTF_PRICES[activeTier]}.00/pc</span>
        </p>
      </div>
    </div>
  );
...
function DTGPricingTable({ qty, activeStyle }: { qty: number; activeStyle: string }) {
  const activeTier = DTG_TIERS.reduce((found, t, i) => (qty >= t.min ? i : found), 0);
  return (
    <div className="mt-3 rounded-xl border border-gray-200 overflow-hidden">
      <div className="bg-gray-50 px-4 py-2.5 flex items-center gap-2 border-b border-gray-200">
        <Table2 size={13} className="text-[#F5C400]" />
...
export default function ProductCustomization({
  productId, is_in_cart = false, onReload,
}: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setVariantId(firstVariant?.id ?? 0);
        setPrice(firstVariant?.price ? Number(firstVariant.price) : Number(data?.price ?? 0));
        setName(data?.name ?? "");
        setProductImageUrl(
          firstVariant?.images?.[0]?.file_uri
            ? `${BASE_URL}${firstVariant.images[0].file_uri}`
            : data?.attachments?.[0]?.file_uri
            ? `${BASE_URL}${data.attachments[0].file_uri}`
            : ""
        );
      } catch (e) {
        console.error(e);
  ...
  const handleWishlist = async () => {
    ...
    try {
      ...
      else { await addToWishlist({ product_id: productId, variant_id: variantId, name, price, image: productImageUrl }); }
      await fetchWishlist();
      onReload?.();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const base64 = await toBase64ViaSameOrigin(productImageUrl);
        if (cancelled) return;
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (dragging === "logo") {
      const bounds = getImageBounds(productImg, CANVAS_SIZE);
      setLogoPos({
        x: Math.max(bounds.x, Math.min(bounds.x + bounds.width - logoSize, pt.x - dragOffset.x)),
        y: Math.max(bounds.y, Math.min(bounds.y + bounds.height - logoSize, pt.y - dragOffset.y)),
      });
  ...
  const handleSelectMaterial = (id: MaterialId) => {
    ...
    if (id === "screenprint" && quantity < 50) setQuantity(50);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 md:divide-x md:divide-gray-100">
          ...
          <div className="p-5 border-t border-gray-100 md:border-t-0">
            ...
            <div className="flex items-center bg-gray-50 rounded-2xl border-2 border-gray-200 overflow-hidden mb-3 focus-within:border-[#F5C400] transition-colors">
              <button onClick={decreaseQuantity} disabled={quantity <= qtyMin}
                className={cn("w-14 h-14 flex items-center justify-center transition-all text-xl font-black flex-shrink-0",
                  quantity <= qtyMin ? "text-gray-300 cursor-not-allowed" : "text-[#F5C400] hover:bg-[#F5C400]/10 active:scale-95"
                )}>
                <Minus size={18} />
              </button>
              <input type="number" min={qtyMin} value={quantity}
                onChange={(e) => setQuantity(Math.max(qtyMin, parseInt(e.target.value) || qtyMin))}
            ...
            <div className="grid grid-cols-4 gap-1.5 mb-3">
              {QTY_PRESETS.map(({ q, hint, accent }) => {
                ...
                return (
                  <button key={q} onClick={() => { if (!isBelowMin) setQuantity(q); }} disabled={isBelowMin}
                    className={cn(
                      "flex flex-col items-center justify-center rounded-xl border-2 py-2.5 px-1 transition-all duration-150",
                      isBelowMin ? "border-gray-100 bg-gray-50 opacity-40 cursor-not-allowed"
                  ...
                  <div className="flex gap-2.5">
                    <button onClick={handleAddToCart}
                      className={cn(
                        ...
                        !allRequirementsMet ? "bg-gray-100 text-gray-400 cursor-not-allowed border-2 border-gray-200"
                          : inCart ? "bg-[#F5C400]/10 text-[#b89000] border-2 border-[#F5C400]/40"
